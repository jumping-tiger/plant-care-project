...
export async function fetchOpenMeteo(lat: number, lon: number): Promise<WeatherInfo | null> {
  ...
  const times = startIdx >= 0 ? allTimes.slice(startIdx) : allTimes;

  const forecast: WeatherForecastDay[] = times.slice(0, 7).map((date: string, i: number) => {
    const realIdx = startIdx >= 0 ? startIdx + i : i;
    return {
      date,
      tempMax: String(Math.round(daily.temperature_2m_max[realIdx])),
      tempMin: String(Math.round(daily.temperature_2m_min[realIdx])),
      textDay: wmoText(daily.weather_code[realIdx]),
    };
  });
