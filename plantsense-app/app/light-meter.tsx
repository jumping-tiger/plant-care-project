...
function getLightLabel(brightness: number): { label: string; advice: string; color: string; emoji: string } {
  if (brightness >= 80) return { label: '强光', emoji: '☀️', color: '#ffb74d', advice: '光线很强，适合喜阳植物（多肉、仙人掌、月季等）。注意避免灼伤叶片。' };
  if (brightness >= 55) return { label: '中等光照', emoji: '🌤', color: '#fff176', advice: '光线充足，大多数室内植物都能良好生长，如绿萝、吊兰等。' };
...
const styles = StyleSheet.create({
  ...
  center: { justifyContent: 'center', alignItems: 'center', backgroundColor: '#0f1f0f' },
  ...
  barBg: { height: 8, backgroundColor: '#1e3e1e', borderRadius: 4, marginBottom: 20, overflow: 'hidden' },
  barFill: { height: '100%', borderRadius: 4 },
  freezeBtn: {
    backgroundColor: '#2d5a27', borderRadius: 14, padding: 14,
    flexDirection: 'row', alignItems: 'center', justifyContent: 'center', gap: 8,
  },
  freezeBtnActive: { backgroundColor: '#1a5a3a' },
  freezeBtnText: { color: '#fff', fontSize: 15, fontWeight: '600' },
  permText: { color: '#888', fontSize: 14, textAlign: 'center', marginVertical: 20 },
  permBtn: { backgroundColor: '#2d5a27', paddingHorizontal: 24, paddingVertical: 12, borderRadius: 20 },
  permBtnText: { color: '#fff', fontSize: 15, fontWeight: '600' },
  backLink: { marginTop: 16 },
  backLinkText: { color: '#4caf50', fontSize: 14 },
  detectingText: { color: '#666', fontSize: 15, textAlign: 'center', paddingVertical: 20 },
});
