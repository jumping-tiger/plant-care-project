...
export default function RegisterScreen() {
  ...
  const handleRegister = async () => {
    ...
    try {
      ...
      Alert.alert('注册成功', '请登录', [{ text: '去登录', onPress: () => router.replace('/(auth)/login') }]);
    } catch (e: any) {
    ...
    <KeyboardAvoidingView style={{ flex: 1 }} behavior={Platform.OS === 'ios' ? 'padding' : undefined}>
      ...
      >
        <View style={styles.logoRow}>
          <MaterialCommunityIcons name="leaf" size={36} color="#4caf50" />
...
const styles = StyleSheet.create({
  ...
  appName: { color: '#4caf50', fontSize: 28, fontWeight: 'bold' },
  form: { width: '100%' },
  ...
  btn: { backgroundColor: '#2d5a27', borderRadius: 12, padding: 16, alignItems: 'center', marginTop: 28 },
